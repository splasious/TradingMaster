...
from app.models.backtest import PortfolioBacktestJob, PortfolioBacktestResult, PortfolioBacktestTrade
from app.models.instrument import Instrument
from app.models.strategy import Strategy, StrategyVersion
from app.models.user import User
from app.schemas.portfolio_backtest import (
    PortfolioBacktestJobCreate,
...
def _job_out(job: PortfolioBacktestJob) -> PortfolioBacktestJobOut:
    return PortfolioBacktestJobOut(
...
@router.post("", response_model=PortfolioBacktestJobOut, status_code=status.HTTP_202_ACCEPTED)
async def create_portfolio_backtest(
    payload: PortfolioBacktestJobCreate,
    ...
    await write_audit_log(
        db, user_id=user.id, action="PORTFOLIO_BACKTEST_STARTED", object_type="strategy", object_id=str(strategy.id),
        new_value={"instrument_count": len(payload.instrument_ids), "timeframe": payload.timeframe},
    )
    await db.commit()
    await db.refresh(job)

    background_tasks.add_task(run_portfolio_backtest_job, job.id)
    return _job_out(job)
...
async def get_portfolio_backtest_result(job_id: str, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)) -> PortfolioBacktestResultOut:
    ...
    if row is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Portfolio backtest result not available yet")
    return PortfolioBacktestResultOut(
        metrics=row.metrics, equity_curve=row.equity_curve,
        instrument_count=row.instrument_count, skipped_symbols=row.skipped_symbols,
    )
...
async def delete_portfolio_backtest(job_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)) -> None:
    job = await db.get(PortfolioBacktestJob, uuid.UUID(job_id))
    if job is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Portfolio backtest job not found")
    ...
    await write_audit_log(
        db, user_id=user.id, action="PORTFOLIO_BACKTEST_DELETED", object_type="portfolio_backtest_job", object_id=str(job.id),
        previous_value={"strategy_id": str(job.strategy_id)},
    )
    await db.delete(job)  # PortfolioBacktestResult/PortfolioBacktestTrade cascade via FK ondelete
    await db.commit()
